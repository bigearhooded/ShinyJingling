...
\textbf{开源}：完整数据集（清洗后 \texttt{pity\_clean.jsonl}，266 条单池样本）+ 代码（\texttt{src/analysis/} 13 个 Python 模块）+ 24 张图（\texttt{figures/}）+ 文档（\texttt{docs/} 8 个 Markdown）+ 本论文 LaTeX 源（\texttt{paper/}），全部已开源至 \texttt{https://github.com/bigearhooded/ShinyJingling}（MIT License，96 文件，约 4.1 MB）。爬虫的 cookie 与 \texttt{data/raw/} 原始评论语料按伦理与平台 ToS 不公开。\\
