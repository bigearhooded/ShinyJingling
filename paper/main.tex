...
\title{\vspace*{0.5cm}\textcolor{violet}{你被官方骗了：\\《洛克王国：世界》异色精灵真实保底机制的\\家族分层贝叶斯识别与最优捕捉策略}\vspace*{0.5cm}}
...
\author{\textbf{大耳帽兜（Big-eared Hooded Bunny）$^{1,*}$}\\
    ...
    \vspace*{0.1cm}
    数据集、代码、所有图表与本论文 LaTeX 源已开源：\texttt{https://github.com/bigearhooded/ShinyJingling}\\
    \normalsize
}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/IMG_5465.jpg}
    \caption{S1「暗夜拾光」赛季 18 个限定异色家族图鉴（贝瑟、大耳帽兜、空空顾、奇丽草、拉特、粉粉星、机械方方、燃渣虫、嗜光嗡嗡、双灯鱼、粉星仔、犀角鸟、治愈兔、恶魔狼、格兰种子、月牙雪熊、呼呼猪、绒绒）。本研究的因果识别目标是这 18 个家族（外加非赛季限定家族）的真实保底机制；§V 给出了样本数 $\ge 4$ 的 top-10 家族级 $\hat p_{0,f}$ 后验。}
    \label{fig:season_shinies}
\end{figure}
...
\begin{quote}
\textit{"洗的人自己看吧，我刷了早就超过 80 个污染了，光是仓库里就有 72 个噩梦，还是没出异色，这官方不仅暗改数据估计还做了两份账，这完全规则说一套机制另一套，纯骗人。"}\\
\hfill —— B 站玩家 rpid=296783806065
\end{quote}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/IMG_5464.jpg}
    \caption{游戏内地图上聚集触发的"噩梦污染体"实时显示（紫色精灵图标 + 橙色噩梦能量光圈）。当玩家在同一区域连续捕捉一定数量的野生精灵后（低阶宠物 $\sim 50$ 只触发 $1$ 个污染），地图上出现污染体；玩家击破或捕捉污染体时按 $p_k$ 概率出货异色。本研究的观测变量 $X = $ "首次出货所需的污染击破次数"即定义在该机制上。}
    \label{fig:map_pollution}
\end{figure}
...
\subsection{游戏内回顾价值的隐式平衡：易出便宜，难出贵}

游戏设计师对家族异质性的隐式平衡，可以从游戏内"异色炫彩回顾结算界面"直接观测到。图~\ref{fig:reward_settle} 给出两个对照家族的截图：

\begin{itemize}
    \item \textbf{大耳帽兜}（图~\ref{fig:reward_settle}a）：在本研究"其他合并"桶中，结算界面显示合计倍率 $\times 6\,000$，获得奖励 $1.08 \times 10^7$ 洛克贝 $\Rightarrow$ 单位回顾值 $v_0 \approx 1\,800$ 洛克贝。
    \item \textbf{恶魔狼}（图~\ref{fig:reward_settle}b）：本研究中最难出家族（$\hat p_{0,f} = 0.080$），结算界面显示合计倍率 $\times 1\,500$，获得奖励 $9.0 \times 10^6$ 洛克贝 $\Rightarrow$ 单位回顾值 $v_0 \approx 6\,000$ 洛克贝。
\end{itemize}

\textbf{关键观察}：恶魔狼的单位回顾值是大耳帽兜的 $\sim 3.3 \times$，恰好抵消其 $1/p_{0,f}$ 比值（$0.17 / 0.080 \approx 2.1\times$）的难度劣势。两个家族的"每小时刷异色期望洛克贝产出"近似相等：
\[
\frac{v_0 \cdot 10}{t_{\text{cycle}}} \;\approx\;
\begin{cases} 1\,800 \cdot 10 / 0.4 = 4.5 \times 10^4 & (\text{大耳帽兜}) \\ 6\,000 \cdot 10 / 0.7 = 8.6 \times 10^4 & (\text{恶魔狼}) \end{cases}
\]
两者同量级（仅 $1.9\times$ 差距），表明游戏设计师有意通过 $v_0$ 的家族异质性平衡 $p_{0,f}$ 异质性。然而该平衡并非完全对称：恶魔狼路线的"小时洛克贝期望"略高，但方差更大（$\hat E[\tau] = 12.8$ vs $6.3$，$\sigma^2$ 比例 $\sim 4 \times$）。\textbf{风险厌恶玩家应优先大耳帽兜路线；风险中性或偏好玩家应优先恶魔狼路线。}此外，奖牌数量（"轻风之息""星月交辉"等成就奖牌）会进一步放大合计倍率，激励玩家长期投入单家族刷取。

\begin{figure}[t]
    \centering
    \begin{minipage}{\linewidth}
        \centering
        \includegraphics[width=0.95\linewidth]{figures/IMG_5459.jpg}\\
        \footnotesize (a) 大耳帽兜 异色炫彩，合计 $\times 6\,000$，奖励 $1.08\times 10^7$ 洛克贝
    \end{minipage}\\[6pt]
    \begin{minipage}{\linewidth}
        \centering
        \includegraphics[width=0.95\linewidth]{figures/IMG_5460.jpg}\\
        \footnotesize (b) 恶魔狼 异色炫彩，合计 $\times 1\,500$，奖励 $9.0\times 10^6$ 洛克贝
    \end{minipage}
    \caption{游戏内"异色炫彩回顾结算"界面截图（4.16 版本）。"了不起天分 $\times 5$" 是 $m_{\text{poll}}$，"异色 $\times 10$" 是 $m_{\text{shiny}}$，"炫彩 $\times 10$" 是棱镜球升格倍数；"轻风之息""完美无瑕"等是奖牌成就额外乘子。难出家族（恶魔狼）的单位回顾值 $v_0 \approx 6\,000$ 是易出家族（大耳帽兜，$v_0 \approx 1\,800$）的 $\sim 3.3\times$，恰好对冲 $1/p_{0,f}$ 难度差，体现游戏设计师"难度--价值"平衡。}
    \label{fig:reward_settle}
\end{figure}
...
表~\ref{tab:balls} 中两项关键价格——\textbf{安妮商店的洛克贝定价机制} 与 \textbf{捕光球的 80\,000 洛克贝/颗交易价}——可以从游戏内截图直接核验（图~\ref{fig:price_evidence}）。

\begin{figure}[t]
    \centering
    \begin{minipage}{\linewidth}
        \centering
        \includegraphics[width=0.95\linewidth]{figures/IMG_5461.jpg}\\
        \footnotesize (a) 商店街\textbf{安妮商店}界面：店主安妮（粉发蓝裙）所售"洛克贝 $\times 2\,000$" 等小额货币包，售价 25 单位；玩家持有 $1.11 \times 10^7$ 洛克贝。
    \end{minipage}\\[6pt]
    \begin{minipage}{\linewidth}
        \centering
        \includegraphics[width=0.95\linewidth]{figures/IMG_5463.jpg}\\
        \footnotesize (b) S1 赛季商店\textbf{捕光球}购买界面：「暗夜拾光」赛季纪念咕噜球，"能十分轻易地捕捉精灵"（即 $q^s = 1.00$）；购买 $26$ 颗合计 $2.08 \times 10^6$ 洛克贝 $\Rightarrow$ 单价 $80\,000$ 洛克贝。
    \end{minipage}
    \caption{表~\ref{tab:balls} 中关键价格的游戏内实测证据。(a) 安妮商店店主与商品列表（致谢段所述赞助方）；(b) 捕光球 $80\,000$ 洛克贝/颗的交易凭证——这是第~VIII 节策略推论中"异色统一用捕光球抓"的成本基线。}
    \label{fig:price_evidence}
\end{figure}
...
\begin{table*}[t]
    \begin{center}
        ...
        \begin{tabular}{rp{0.20\textwidth}p{0.65\textwidth}}
            ...
            $8$ & 选高星光值精灵刷池，优先易出家族（贝瑟、方方、雪熊、柴渣，$\hat E[\tau] < 9$） & 缩短野怪触发污染时间；§V 分层后验显示家族 $p_{0,f}$ 跨度 $0.08$--$0.17$，易出家族日产出比难出家族高 $\sim 30\%$（表~\ref{tab:family}） \\
            $9$ & 高级球（$12$k）仅在采花跟不上时备用；难出家族（兔、狼、恶魔狼）需更长 $t_{\text{cycle}}$，务必并行多池摊销方差 & 时间预算 LP 显示 S2 仅在 $T_h \ge 16$ h 撞 PVP 上限时才略优 \\
            \hhline{===}
        \end{tabular}
    \end{center}
...
\subsection{$p_0$ 的部分识别与自报权重鲁棒性}

M1 与 M2/M3 给出的 $\hat p_0$ 不同（$0.092$ vs $0.062$），反映"高 $p_0$ + 弱偏差"与"低 $p_0$ + 强偏差"两条路径都能拟合数据；LOO 选 M1 因为复杂度更低。但 \textbf{综合出货率} $1/\hat E[\tau]$ 跨模型一致集中在 $[0.06, 0.09]$，构成对官方 $0.02$ 的鲁棒拒绝。(\ref{eq:wk}) 的 4 段自报权重已捕捉主要 U 形结构，但简化于现实：中段 $30$--$77$ 内可能存在缓慢渐变（当前 $b_{\text{low}}$ 仅常数）；凑整偏好未建模；平台异质性（B 站 vs 小红书）使外部有效性受限。

\subsection{若强行假设官方诚实，及星光值的解耦}

若假设 $p_0 = 0.018$ 严格成立，自报权重需给出 $\ge 5\times$ 的中段抑制才能解释观测——要求"$30$--$77$ 段玩家几乎完全沉默"，与 B 站评论 $13\%$ 中段报告比例不符。鲁棒结论仍是"自报偏差真实且数量级 $5\times$ 左右"。游戏机制中"星光值"只影响每只精灵触发污染的速度（即时间预算 $T_n$），不影响 $p_k$，本研究将其与概率识别解耦。

\subsection{局限与未来工作}

主要局限：（i）单平台数据（仅 B 站，缺 XHS/贴吧/TapTap 对照）；（ii）LLM 抽取无人工金标交叉验证；（iii）自报权重 4 段简化未捕捉中段渐变与凑整效应；（iv）表~\ref{tab:balls} 中各球 $q_{\text{cap}}$ 是社区共识；（v）数据看似硬保底亦可能是软保底窗口太窄 + 自报偏差抹平的混淆；（vi）表~\ref{tab:cross_gacha} 横向对比的其他游戏 $E[\tau]$ 未做同等贝叶斯识别。未来工作包括跨平台交叉验证、人工金标录播、扩展同等识别框架到原神/FGO/碧蓝档案以建立抽卡透明度评级体系，以及家族级星光值--污染产出速度的具体建模。
...
\begin{enumerate}
    \item \textbf{综合概率被强烈拒绝、硬保底是真实机制}：三模型综合出货率后验集中在 $[0.06, 0.09]$，是官方 $0.02$ 的 \textbf{3--4.5 倍}（$P(p_0 < 0.0143) = 0\%$）；M1 LOO 严格主导（$\Delta\text{elpd} > 13$，M2 退化为 $k^* = 78$、M3 退化为 $\eta = 19.3$），民间"70--75 冲刺大法"统计无效；
    \item \textbf{家族异质性显著}：分层 Beta-Binomial 给出家族级 $\hat p_{0,f} \in [0.08, 0.17]$，最易出（贝瑟）与最难出（恶魔狼）相差 $2\times$，每日异色产出差异 $\pm 30\%$；游戏内 $v_0$ 异质性恰好对冲该差异（恶魔狼 $v_0 \approx 6\,000$ 是大耳帽兜 $\approx 1\,800$ 的 $3.3\times$）；
    \item \textbf{时间预算 LP 三档玩家 SOP}：占优策略恒为"属性球抓污染 + 普通球摔野怪 + 单颗 $80$k 捕光球抓异色"；单轮耗时（$\sim 0.5$ h/异色）始终是绑定约束；产出 $0.85 / 5.1 / 17$ 异色/天 (轻/中/重度)；棱镜球升格收藏溢价需 $\phi > 640$ 才回本，氪金不合理；
    \item \textbf{抽卡谱定位}：本作位于"低 $E[\tau] + $ 严硬保底"友善区，玩家长期获取效率优于原神 / FGO，但官方公示透明度被证伪——\textbf{诚实度 $\ne$ 友好度，此案例是后者真实更高的反向欺骗}。
\end{enumerate}

方法论贡献：自报抽卡数据必须做严格选择偏差校正；4 段自报权重框架可推广，并支持以贝叶斯严谨度建立抽卡透明度评级体系。


\section{MATERIALS AND METHODS / 材料与方法}

\textbf{数据 + 模型}：B 站 \texttt{get\_comments\_lazy} API 抽 30 视频 $\times$ 5 关键词 $\approx 26$K 评论；正则粗筛 + 嵌入召回 + codex GPT-5.4 二次结构化抽取 + 手工清洗 $\Rightarrow$ 266 条单池。PyMC 5.25.1\cite{ref_pymc} NUTS 4 chains $\times$ 1\,500 draws，$\texttt{target\_accept}=0.92$；似然由 (\ref{eq:pmf}) $\times$ (\ref{eq:wk}) 归一化的 Categorical 给出；LOO 用 ArviZ \texttt{az.compare()}\cite{ref_loo}；家族分层在相同框架上加 Beta 超先验 partial pooling\cite{ref_self_report_bias}；经济仿真按 (\ref{eq:expected_profit}) 数值积分；时间预算 LP 网格步长 0.05；多池用边际 $p_k$ 贪心。\textbf{开源}：完整工件（数据 + 13 模块代码 + 30 张图 + 8 个 markdown 文档 + LaTeX 源）见 \texttt{https://github.com/bigearhooded/ShinyJingling}（MIT 许可；爬虫 cookie 与原始评论按平台 ToS 不公开）。\\

ACKNOWLEDGEMENT / 致谢\\
本研究由\textbf{安妮（商店街安妮商店店长，见图~\ref{fig:price_evidence}a）}全额赞助。安妮女士在 4.16 版本主动将高级球价格由 15\,000 洛克贝下调至 12\,000 洛克贝以应对远行商人的市场竞争，使得本研究中"高级球抓污染"路线的经济仿真得以基于真实交易价格——这一价格调整是表~\ref{tab:balls} 与第~VIII 节策略推论的基础常数。安妮商店位于商店街（地图西南方向入口可直接传送），同时经营消耗品、技能石、魔法粉尘及隐藏精灵幻境入口（"小夜"获取链路）。此外感谢 B 站玩家社区对异色保底机制的开放讨论，使本研究的自报数据采集成为可能；感谢魔法学院全域魔法传送阵系统的稳定运行，使得多池并行实验中的家族池切换零延迟。计算资源由洛克王国魔法学院"分层贝叶斯"研究组提供。LLM 抽取使用 OpenAI GPT-5.4（codex）。S.H.*.T Journal 反串学术风格启发了本文的标题与组织。
...
\bibitem{ref_v2_design} 大耳帽兜（Big-eared Hooded Bunny）, ``异色狩猎研究设计 V2，'' 项目内部文档 \texttt{docs/01\_research\_design.md}（2026 年 4 月 21 日）。
